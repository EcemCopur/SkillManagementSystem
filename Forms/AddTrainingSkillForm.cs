...
{
    public partial class AddTrainingSkillForm : Form
    {
        ...
        public AddTrainingSkillForm(DataManager manager, Training train)
        {
            dataManager = manager;
            training = train;
            InitializeComponent();
        ...
        private void InitializeCustomComponents()
        {
            ...
            var existing = dataManager.TrainingSkills.Where(ts => ts.TrainingId == training.Id).Select(ts => ts.SkillId).ToList();
            cmbSkill.DataSource = dataManager.Skills.Where(s => !existing.Contains(s.Id)).ToList();
            this.Controls.Add(cmbSkill);
            y += vs;

            AddLabel("Target Level:", lm, y);
            ...
            btnSave.Click += (s, e) =>
            {
                ...
                dataManager.TrainingSkills.Add(new TrainingSkill { TrainingId = training.Id, SkillId = (int)cmbSkill.SelectedValue, TargetLevel = (int)cmbLevel.SelectedItem });
        ...
        private void InitializeComponent()
        {
            ...
            this.Name = "AddTrainingSkillForm";
