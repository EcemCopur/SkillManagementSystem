...
namespace SkillManagementSystem.Forms
{
    public partial class EmployeeEditForm : Form
    {
        ...
        private void InitializeCustomComponents()
        {
            ...
            // Only allow digits
            txtIdentityNumber.KeyPress += (s, e) =>
            {
            ...
            // Initialize positions for the first department if not in edit mode
            if (!isEditMode && cmbDepartment.Items.Count > 0)
            {
        ...
        // FIX: Check if SelectedItem is a Department object instead of SelectedValue
        private void CmbDepartment_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbDepartment.SelectedValue is int deptId)
            {
                ...
                // In edit mode, set the current position after loading department positions
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                employee.IdentityNumber = txtIdentityNumber.Text.Trim();
        ...
        private bool ValidateInput()
        {
            ...
            // Validate identity number has exactly 11 digits
            if (string.IsNullOrWhiteSpace(txtIdentityNumber.Text))
            {
                MessageBox.Show("Identity number is required.", "Validation Error",
            ...
            {
                MessageBox.Show("Identity number must be exactly 11 digits.", "Validation Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtIdentityNumber.Focus();
                return false;
            }

            if (!txtIdentityNumber.Text.All(char.IsDigit))
            {
                MessageBox.Show("Identity number must contain only digits.", "Validation Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtIdentityNumber.Focus();
                return false;
            }
            ...
            if (cmbPosition.SelectedIndex == -1 || cmbPosition.Items.Count == 0)
            {
                MessageBox.Show("Please select a position. Make sure the selected department has positions.",
